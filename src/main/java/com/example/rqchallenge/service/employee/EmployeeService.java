...
public class EmployeeService implements IEmployeeService {
	...
	public List<Employee> getEmployeeByNameSearch(String searchString) {
		logger.error("Service Endpoint for getEmployeeByNameSearch():",
				Constants.EMPLOYEES + Constants.SEPARATOR + searchString);
	...
	public Integer getHighestSalaryOfEmployees() {
		...
		return getAllEmployees().stream().max(Comparator.comparing(Employee::getEmployeeSalary)).get()
				.getEmployeeSalary();
	}
	...
	public List<String> getTopTenHighestEarningEmployeeNames() {
		...
		return getAllEmployees().stream().sorted(Comparator.comparing(Employee::getEmployeeSalary).reversed()).limit(10)
				.flatMap(employee -> Stream.of(employee.getEmployeeName())).collect(Collectors.toList());
	}
